...
export default function CandidatesPage() {
  ...
  // Voice Ingest Profile Selection - pre-select from URL if provided
  const [selectedProfileId, setSelectedProfileId] = useState<string | null>(profileFromUrl);
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // Pass voice ingest profile ID if selected
      if (selectedProfileId) {
        formData.append("job_profile_id", selectedProfileId);
      }
        ...
        {step === "upload" && (
          <div className="animate-fade-in space-y-12">
            ...
            <div className="max-w-4xl mx-auto grid grid-cols-1 lg:grid-cols-5 gap-8">
              ...
              <div className="lg:col-span-5 mb-2">
                <h3 className="font-medium mb-4 flex items-center gap-2 text-sm text-gray-400 uppercase tracking-wider">
                  <span className="w-5 h-5 rounded-full bg-indigo-500/20 flex items-center justify-center text-xs text-indigo-300">0</span>
              ...
              <div className="lg:col-span-2 glass-panel p-1 rounded-3xl flex flex-col h-full hover:bg-white/5 transition-colors group">
                <div className="p-6 h-full flex flex-col">
                  <h3 className="font-medium mb-4 flex items-center gap-2 text-sm text-gray-400 uppercase tracking-wider">
                    1. Job Context {selectedProfileId && <span className="text-xs text-indigo-300">(optional with profile)</span>}
                  </h3>
                  <textarea
                    value={jobDescription}
                    onChange={(e) => setJobDescription(e.target.value)}
                    placeholder={selectedProfileId ? "Profile will be used. Add extra context here if needed..." : "Paste JD here..."}
            ...
            <div className="max-w-md mx-auto">
              {/* If profile selected, allow direct upload without JD analysis */}
              {selectedProfileId ? (
                <button
                ...
                <div className="mt-6 pt-6 border-t border-white/5">
                  {!showAddFieldForm ? (
                    <button
                      ...
                      className="w-full py-3 rounded-xl bg-white/5 border border-dashed border-white/20 text-white/50 hover:text-white/80 hover:border-white/40 text-sm font-medium transition-all flex items-center justify-center gap-2"
                    >
                    ...
                    <div className="space-y-4 p-4 rounded-xl bg-white/5 border border-white/10">
                      <div className="flex items-center justify-between">
                        <h4 className="text-sm font-medium text-white">Add Custom Extraction Field</h4>
                        <button
                          onClick={() => {
                            setShowAddFieldForm(false);
                            setNewFieldName("");
                      ...
                      {/* Field Name */}
                      <div>
                        <label className="block text-xs text-white/50 mb-1.5">Field Name *</label>
                        <input
                          value={newFieldName}
                          onChange={(e) => setNewFieldName(e.target.value)}
                          placeholder="e.g., enterprise_sales_experience"
                      ...
                      {/* Field Description */}
                      <div>
                        <label className="block text-xs text-white/50 mb-1.5">Description (what to extract)</label>
                        <textarea
                          value={newFieldDescription}
                          onChange={(e) => setNewFieldDescription(e.target.value)}
                          placeholder="e.g., Has the candidate sold to enterprise companies with 1000+ employees?"
                          rows={2}
                      ...
                      {/* Field Type */}
                      <div>
                        <label className="block text-xs text-white/50 mb-1.5">Field Type</label>
                        ...
                        <p className="text-[10px] text-white/30 mt-1.5">
                          {newFieldType === "boolean" && "Yes/No answer (e.g., has_startup_experience)"}
                          {newFieldType === "number" && "Numeric value (e.g., years_of_experience, deal_size)"}
                          {newFieldType === "string" && "Text answer (e.g., primary_industry, sales_methodology)"}
                          {newFieldType === "string_list" && "Multiple values (e.g., skills, certifications)"}
                        </p>
                      </div>
                      ...
                      <button
                        ...
                        className="w-full py-2.5 rounded-lg bg-blue-600 text-white text-sm font-medium hover:bg-blue-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        Add Field
                      </button>
                    </div>
